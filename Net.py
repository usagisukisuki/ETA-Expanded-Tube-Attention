...
class U_Net(nn.Module):
    def __init__(self, output=2):
        ...
        self.conv101 = nn.Conv2d(64,64,3,1,1)
        self.conv102 = nn.Conv2d(64,64,3,1,1) 
        self.conv103 = nn.Conv2d(64,64,3,1,1) 
        self.conv104 = nn.Conv2d(64,64,3,1,1)
        self.conv105 = nn.Conv2d(64,output,1,1)
        self.conv101l = nn.Conv2d(64,output,1,1)
        self.conv102l = nn.Conv2d(64,output,1,1) 
        self.conv103l = nn.Conv2d(64,output,1,1) 
        self.conv104l = nn.Conv2d(64,output,1,1) 
        ...
        self.bn103 = nn.BatchNorm2d(64)
        self.bn104 = nn.BatchNorm2d(64)
    ...
    def forward(self,x):
        ...
        # attention 
        h1 = F.relu(self.bn101(self.conv101(h)))
        h1_t1 = self.conv101l(h1)
        h1_t2 = torch.sigmoid(h1_t1)
        h1 = h1*h1_t2# + h1
        
        h2 = F.relu(self.bn102(self.conv102(h1)))
        h2_t1 = self.conv102l(h2)
        h2_t2 = torch.sigmoid(h2_t1)
        h2 = h2*h2_t2# + h2
        
        h3 = F.relu(self.bn103(self.conv103(h2)))
        h3_t1 = self.conv103l(h3)
        h3_t2 = torch.sigmoid(h3_t1)
        h3 = h3*h3_t2# + h3
        
        h4 = F.relu(self.bn104(self.conv104(h3)))
        h4_t1 = self.conv104l(h3)
        h4_t2 = torch.sigmoid(h4_t1)
        h4 = h4*h4_t2# + h3
        
        h5 = self.conv105(h4)

        return h1_t1, h2_t1, h3_t1, h4_t1, h5, h1_t2, h2_t2, h3_t2, h4_t2
